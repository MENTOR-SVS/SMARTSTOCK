...
BACKUP_LOG_PATH = os.path.join(DATA_PATH, "inventory_validation_log backup.csv")
...
selected_tab = st.sidebar.radio(
    "Select a Module:",
    ["📈 Demand Forecast", "⚠️ Stock Alerts", "📂 Data Management", "📑 Inventory Logs"]
)
...
if selected_tab == "📈 Demand Forecast":
    ...
    if not forecast_df.empty and set(["ds", "yhat"]).issubset(forecast_df.columns):
        ...
        category_col = next((col for col in possible_category_cols if col in forecast_df.columns), None)
...
# ---------------- ✅ NEW Module 4: Inventory Logs ----------------
elif selected_tab == "📑 Inventory Logs":
    st.header("📑 Inventory Validation Logs")
    st.markdown("""
    Download historical **inventory validation logs** containing EOQ, Reorder Points, and other operational metrics.
    This helps maintain transparency and traceability for each update cycle.
    """)

    if os.path.exists(BACKUP_LOG_PATH):
        try:
            log_df = pd.read_csv(BACKUP_LOG_PATH)
            st.success(f"✅ Found log file: `{os.path.basename(BACKUP_LOG_PATH)}`")

            # Quick Stats
            st.metric("Total Log Entries", log_df.shape[0])
            if 'EOQ' in log_df.columns:
                st.metric("Average EOQ", round(log_df['EOQ'].mean(), 2))
            if 'ROP' in log_df.columns:
                st.metric("Average ROP", round(log_df['ROP'].mean(), 2))

            st.dataframe(log_df.head(10), use_container_width=True)

            csv_bytes = log_df.to_csv(index=False).encode('utf-8')
            st.download_button(
                label="📥 Download Inventory Log Backup",
                data=csv_bytes,
                file_name=f"inventory_validation_log_backup_{pd.Timestamp.now().strftime('%Y%m%d')}.csv",
                mime="text/csv",
                use_container_width=True
            )

            with st.expander("Show Column Details"):
                st.write(f"Total Columns: {log_df.shape[1]}")
                st.write("Columns:", list(log_df.columns))
        except Exception as e:
            st.error(f"Error loading backup log: {e}")
    else:
        st.warning("⚠️ No backup log file found in the `data/` directory.")
